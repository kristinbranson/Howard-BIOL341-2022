...
if usesampledata,
  ...
  for i = 1:numel(expdirs),
    ...
    trxfile = fullfile(expdirs{i},'processed_trx.mat');
    if ~exist(trxfile,'file'),
      error('Experiment directory %s is not post-processed yet.',expdirs{i});
    end
  end
...
if ~isempty(data.summary.activation),
  ...
  hax = PlotHistogramOnVsOff(data,'speed_mmps',...
   ...
   data = HistogramFeatureOverVideo(data,'speed_mmps',...
     'nbins',nbins,...
     'binlimits',binlimits_speed);
   hax = PlotHistogramAll(data,'speed_mmps',...
     'featlabel','Speed (mm/s)');
...
nbins = 25;
binlimits_dist2fly = [0,maxdist2fly_plot];
...
if ~isempty(data.summary.activation),
  ...
  hax = PlotHistogramOnVsOff(data,'dist2fly_mm',...
   ...
   data = HistogramFeatureOverVideo(data,'dist2fly_mm',...
     'nbins',nbins,...
     'binlimits',binlimits_dist2fly);
   hax = PlotHistogramAll(data,'dist2fly_mm',...
     'featlabel','Inter-fly dist. (mm)');
